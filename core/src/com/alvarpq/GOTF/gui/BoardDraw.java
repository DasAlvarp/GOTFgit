...
public class BoardDraw 
{
	...
	public void myBoard(int row)
	{
		...
		for(int x = 0; x < row; x++)
		{
			if(x %2 == 0)
			{
				makeRow(0, hOff * x, us,c);
			}
			else
			{
				makeRow((int)LENGTH / 2, hOff * x, us,c);
	...
	public void theEnemy(int row)
	{
		...
		for(int x = 0; x < row; x++)
		{
			if(x % 2 == 1)
			{
				makeRow((int)(LENGTH * 4), hOff * x, them,c);
			}
			else
			{
				makeRow((int)(LENGTH / 2 + LENGTH * 4), hOff * x, them, c);
	...
	//draws a row.
	private void makeRow( int startX, int startY, Hex[][] arr, int row)//length in hexes
	{
