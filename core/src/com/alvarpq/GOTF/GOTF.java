...
public class GOTF extends ApplicationAdapter
{
	...
	public void render()
	{
		...
		lastTime = System.currentTimeMillis();
		Gdx.gl.glClearColor((float)Math.abs(Math.sin(timePassed/60000.0*Math.PI*2)), (float)Math.abs(Math.cos(timePassed/60000.0*Math.PI*2)), (float)Math.abs(Math.tan(timePassed/60000.0*Math.PI*2)), 1);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
		batch.begin();
